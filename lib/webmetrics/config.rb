...
module Webmetrics
  ...
  module Config
    ...
    option :user_collection_name, :default => "webmetrics_users"
    option :event_collection_name, :default => "webmetrics_events"
    ...
    # Get the Mongo::Connection to use to pull the Webmetrics metrics data
    def connection
      @connection || Mongo::Connection.new
    end

    # Set the Mongo::Connection to use to pull the Webmetrics metrics data
    def connection=(connection)
      @connection = connection
    end

    # Get the Mongo::Database associated with the Webmetrics metrics data
    def database
      @database || connection.db(database_name)
    end

    # Set the Mongo::Database associated with the Webmetrics metrics data
